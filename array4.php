...
foreach ($arr as  $key => $val ) {
    $result = get_money_count($val['rank']);
   $arr[$key]['moneyCount'] = $result['money'];
   $arr[$key]['orderCount'] = $result['order'];
}

function get_money_count($rank) {
    $money = 0;
    $order = 0;
...
//4, 新增一个MoneyCount键和orderCount键，计算rank数组里的MoneyCnt和orderSum进行求和
